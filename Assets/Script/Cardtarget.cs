...

{
    ...
    public void CardBattleTrue(GameObject CardGameObject)
    {
        ...
        {//베틀중이 아니고, 클릭한 카드의 코스트가 본인의 코스트보다 낮을 경우 / 클릭한 카드가 있을경우(클릭한 카드가 있을 경우에 코스트를 확인하는 것을 방지하지 위해) / 배틀중일 경우(코스트를 확인하는 것을 방지하지 위해)
            ...
            {
                case "PlayerCard":
                    if (PlayerBattlePage == false && MouseDrag != null && CardGameObject.transform.parent.tag == "EnemyCardZone" && MouseDrag.GetComponent<Card>()._CardDataList[0].cardType != CardType.MonsterCard)
